...
class Auth extends BD_Controller {
    ...
    public function login_post()
    {
        ...
        $val = $this->M_main->get_surveyor($q)->row(); //Model to get single data row from database base on email
        if($this->M_main->get_surveyor($q)->num_rows() == 0){$this->response($invalidLogin, REST_Controller::HTTP_NOT_FOUND);}
		$match = $val->password;   //Get password for user from database
        if($p == $match){  //Condition if password matched
        	$token['id'] = $val->surveyor_id;  //From here
